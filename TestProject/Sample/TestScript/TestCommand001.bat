...
rem # 通常どおりのGETリクエスト
curl -X GET %SERVER_URL%/api/profile/list | jq

rem # 何も指定しないPOSTリクスト
curl -X POST %SERVER_URL%/api/profile/list | jq

rem # 値を渡すPOSTリクエスト (Refresh = true)
curl -X POST %SERVER_URL%/api/profile/list ^
  -d "refresh=true" | jq
...
curl -X POST %SERVER_URL%/api/profile/delete ^
...
rem # ※Protectedプロファイルは除外
curl -X DELETE %SERVER_URL%/api/profile/delete ^
  -d "all=true" | jq
...
curl -X GET %SERVER_URL%/api/user/session | jq

rem # ログイン中セッションの一覧を取得(POSTメソッド)
curl -X POST %SERVER_URL%/api/user/session | jq

rem # ログイン中セッションの一覧を取得 (Refresh = false)
curl -X POST %SERVER_URL%/api/user/session ^
  -d "refresh=false" | jq
...
rem # 指定したユーザーでログオン
curl -X POST %SERVER_URL%/api/user/logon ^
...
curl -X GET %SERVER_URL%/api/user/logoff | jq

rem # 全セッションをログオフ (POSTメソッド)
curl -X POST %SERVER_URL%/api/user/logoff | jq

rem # 指定したユーザーのセッションをログオフ (POSTメソッド, Administratorのみログオフ)
curl -X POST %SERVER_URL%/api/user/logoff ^
...
curl -X GET %SERVER_URL%/api/user/disconnect | jq

rem # 全RDPセッションを切断 (POSTメソッド)
curl -X POST %SERVER_URL%/api/user/disconnect | jq

rem # 指定したRDPセッションを切断 (POSTメソッド, Administratorのみ切断)
curl -X POST %SERVER_URL%/api/user/disconnect ^
  -d "username=Administrator" | jq
...
rem # =========================================================
rem # ログを取得 (GET, 最後10行)
curl -X GET %SERVER_URL%/api/log/print | jq

rem # ログを取得 (POST, 最後10行)
curl -X POST %SERVER_URL%/api/log/print | jq
...
curl -X GET %SERVER_URL%/api/server/info | jq

rem # マシン情報の取得 (POSTメソッド)
curl -X POST %SERVER_URL%/api/server/info | jq
...
curl -X GET %SERVER_URL%/api/server/network | jq

rem # ネットワークアドレス情報の取得 (POSTメソッド)
curl -X POST %SERVER_URL%/api/server/network | jq

rem # ネットワークアドレス情報の取得 (POSTメソッド, Refresh = true)
curl -X POST %SERVER_URL%/api/server/network ^
  -d "refresh=true" | jq
...
rem # =========================================================
rem # サーバ終了 (GETメソッド)
curl -X GET %SERVER_URL%/api/server/close | jq

rem # サーバ終了 (POSTメソッド)
curl -X POST %SERVER_URL%/api/server/close | jq
